...
export const job = createSlice({
  ...
  reducers: {
    ...
    addLanguageFilters: (state, action) => {
      const isAlreadyPresent = state.LanguageFilters.find(
    ...
    addRolesFilters: (state, action) => {
      const isAlreadyPresent = state.RolesFilters.find(
        (el) => el.name === action.payload
      );
      if (!isAlreadyPresent) {
    ...
    addLevelFilters: (state, action) => {
      const isAlreadyPresent = state.LevelFilters.find(
        (el) => el.name === action.payload
      );
      if (!isAlreadyPresent) {
    ...
    addToolsFilters: (state, action) => {
      const isAlreadyPresent = state.ToolsFilters.find(
        (el) => el.name === action.payload
      );
      if (!isAlreadyPresent) {
    ...
    removeRoleFilters: (state, action) => {
      state.RolesFilters = state.RolesFilters.filter(
        (el) => el.id !== action.payload
      );
    },
    removeLevelFilters: (state, action) => {
      state.LevelFilters = state.LevelFilters.filter(
        (el) => el.id !== action.payload
      );
    },
    removeToolsFilters: (state, action) => {
      state.ToolsFilters = state.ToolsFilters.filter(
        (el) => el.id !== action.payload
      );
    },
...
export function getJobList(action) {
  return function (dispatch, getState) {
    fetch("https://github.com/Bezies/Job-listings/blob/main/src/assets/data.json")
      .then((res) => res.json())
